...
router.get("/manage_home_page",async function(req,res){
...
router.get("/manage_about_page",async function(req,res){
...
router.get("/manage_certificate_page",async function(req,res){
    var sql=`SELECT * FROM certificate_page`;
    var data=await exe(sql);
    var obj={"certificate":data[0]}
    res.render("admin/manage_certificate_page.ejs",obj)
});
router.post("/update_certificate_page", async function(req,res){
    var d=req.body;
    if(req.files)
    {
        var file_name=new Date().getTime()+".png";
